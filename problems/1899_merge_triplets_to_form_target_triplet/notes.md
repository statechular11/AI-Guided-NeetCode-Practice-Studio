# Notes - 1899. Merge Triplets to Form Target Triplet

## Core Idea

Discard any triplet that exceeds target in any coordinate; valid triplets only need to cover each target coordinate.

## Reference Solutions

| File | Role | Complexity |
| --- | --- | --- |
| `solution_filter_and_cover.py` | primary greedy solution | Time: O(n); Space: O(1) |
| `solution_coordinate_max.py` | alternative greedy reference | Time: O(n); Space: O(1) |

## Pitfalls To Watch

- State the greedy choice and why earlier/later choices cannot improve it.
- Watch boundary cases where equality is allowed.
- For reachability problems, track the farthest possible boundary.
- For string balance problems, a range of possibilities can be more useful than one exact state.

## Review Log

Add review observations here after NeetCode Prep Coach reviews your `solution.py`.
