"""
1448. Count Good Nodes in Binary Tree - Dfs Carrying Maximum Value Seen So Far
...
Core idea:
    ...
    This specific variant uses: DFS carrying maximum value seen so far.
...
Walkthrough:
    On the local case `example_1`, the input is `{"root": [3, 1, 4, 3, null, 1, 5]}` and the expected result is `4`. Trace how the reference's state changes until that expected result is forced.
...
class Solution:
    def goodNodes(self, root: TreeNode) -> int:
        """Count nodes that are at least the max value seen on their path.

        Carry the maximum ancestor value while descending. A node is good if its
        value is >= that maximum, then it becomes the new maximum for children.
