"""
104. Maximum Depth of Binary Tree - Recursive Max Child Depth
...
Core idea:
    ...
    This specific variant uses: recursive max child depth.
...
Walkthrough:
    On the local case `example_1`, the input is `{"root": [3, 9, 20, null, null, 15, 7]}` and the expected result is `3`. Trace how the reference's state changes until that expected result is forced.
...
class Solution:
    def maxDepth(self, root: Optional[TreeNode]) -> int:
        """Depth is one plus the deeper child depth.

        An empty subtree has depth 0. Recursing down both children gives the
        maximum root-to-leaf node count.
